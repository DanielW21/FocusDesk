...
import { taskIsCompletedOnDate, type CalendarEvent, type Task } from "./model";
...
export function renderTaskList(tasks: Task[], selectedDate: string): string {
  ...
  return `<div class="task-list">${tasks
    .map((task) => {
      const completed = taskIsCompletedOnDate(task, selectedDate);
      const recurrence = task.recurrence;
      const recurring = recurrence !== null && recurrence !== undefined;
      const taskDate = recurring
        ? `${recurrence.type === "daily" ? "Daily" : "Weekly"} · ${formatShort(recurrence.start)}`
        : task.date === selectedDate
          ? task.time || ""
          : formatShort(task.date);
      return `<div class="task ${completed ? "done" : ""} ${recurring ? "recurring-task" : ""}" data-id="${task.id}">
      <button type="button" class="check" data-action="toggle-task"${recurring ? ` data-occurrence-date="${selectedDate}"` : ""} aria-label="Toggle task">${completed ? "✓" : ""}</button>
      <span class="priority-dot ${task.priority ?? "normal"}"></span>
      <button type="button" class="task-title" data-action="edit-task">${escapeHtml(task.title)}</button>
      <span class="tag">${escapeHtml(recurring ? "Routine" : task.tag || "Task")}</span>
      <span class="task-time">${escapeHtml(taskDate)}</span>
      <button type="button" class="delete" data-action="delete-task" aria-label="Delete task">×</button>
    </div>`;
    })
    .join("")}</div>`;
}
  ...
  const tasks = context.state.tasks
    .filter((task) =>
      context.taskFilter === "active"
        ? !taskIsCompletedOnDate(task, context.selectedDate)
...
export function renderCalendarView(context: WorkspaceViewContext): string {
  ...
  for (let day = 1; day <= days; day += 1) {
    ...
    const tasks = context.state.tasks.filter(
      (task) => task.recurrence == null && task.date === date,
    );
    cells += `<div class="day-cell ${date === localDate(context.now) ? "today-cell" : ""}"><button type="button" class="day-num ${date === localDate(context.now) ? "today-num" : ""}" data-action="select-date" data-date="${date}">${day}</button>${events.map((event) => `<div class="cal-event ${event.source === "focusdesk" ? "focusdesk-event" : "google-event"}"${calendarEventStyle(event, context.googleCalendarCalendars)} ${event.source === "focusdesk" ? `data-action="edit-calendar-event" data-id="${event.id}" role="button" tabindex="0" title="Edit ${escapeHtml(event.title)}"` : `title="${escapeHtml(event.title)} · read-only"`}>${escapeHtml(event.title)}</div>`).join("")}${tasks.map((task) => `<div class="cal-event task-event">${taskIsCompletedOnDate(task, date) ? "✓ " : ""}${escapeHtml(task.title)}</div>`).join("")}</div>`;
  }
