  ...
  for (const [index, jobRow] of jobRows.entries()) {
    ...
    if (!org || !jobTitle) {
      ...
      const explicitlyUnavailable =
        /access denied|not authorized|not permitted|posting (?:is )?no longer available|posting (?:has been )?removed/i.test(
          detailText,
        );
      console.warn(
        JSON.stringify({
          event: "posting-skipped",
          jobId: jobRow.id,
          classification: explicitlyUnavailable
            ? "details-unavailable"
            : "unrecognized-layout",
          missingOrganization: !org,
          missingJobTitle: !jobTitle,
        }),
      );
      progressReporter.reportProgress(
        `Checked job ${index + 1} of ${jobRows.length}`,
      );
      continue;
    }
    ...
    scrapedJob = mapToScrapedJob(qualifierMap, scrapedJob, "Qualifier");
    scrapedJob.categorizations = scrapedJob.categorizations ?? {};
    scrapedJob.categorizations.specialRequirements =
      scrapedJob.categorizations.specialRequirements ?? [];
    for (const [key, value] of Object.entries(qualifierMap)) {
      const qualifier = `${key}: ${value}`
        .replace(/<[^>]+>/g, " ")
        .replace(/\s+/g, " ")
        .trim();
      if (qualifier)
        scrapedJob.categorizations.specialRequirements.push(qualifier);
    }
    scrapedJob.location = {
      address:
